...
# 原第 3—30 行是同一段多行字符串；为保留其原始内容，将解释集中放在这里。
# [原第 3 行] 三个双引号开始一个多行字符串。第 3—30 行整体是说明文字，里面的伪代码和安装命令不会执行。严格说它位于 future 导入之后，并不是模块的第一条语句，因此不会成为模块的 __doc__。
# [原第 4 行] 说明文字中的标题：s01_agent_loop.py，意为“第 1 课：智能体循环”。这是文本，不是变量赋值或函数调用。
# [原第 6 行] 说明文字：用一种核心循环模式说明 AI 助手如何工作。这里的“全部秘密”是教学概括，后面的实现仍只是简化示例。
# [原第 8 行] 伪代码：只要当前轮还有工具调用，就继续循环。这里不是合法的实际 Python while 语句；真正的有界循环在第 239 行。
# [原第 9 行] 伪代码：把消息历史 messages 和工具说明 tools 交给大语言模型 LLM，取得响应 response。
# [原第 10 行] 伪代码：由本地程序执行模型请求的工具。模型返回请求，并不是它自己直接运行这段 Python。
# [原第 11 行] 伪代码：把工具结果追加到消息历史，供下一次模型调用读取。
# [原第 13 行] ASCII 流程图的上边框。加号和横线在这个多行字符串中只是用于画图。
# [原第 14 行] 图中的箭头表示“用户 → 大语言模型 → 工具”。User 是用户，LLM 是大语言模型，Tool 是工具。
# [原第 15 行] prompt 表示用户输入；execute 表示执行工具。这些文字只用于解释流程。
# [原第 16 行] ASCII 方框的下边框及向下连接线，不参与运行。
# [原第 17 行] 箭头向上表示结果回到模型；右侧竖线表示工具执行后的返回路径。
# [原第 18 行] tool_result 表示工具结果，沿着连线交给下一次模型调用。
# [原第 19 行] 底部连线把工具结果接回模型，形成反馈循环。
# [原第 20 行] loop continues 意为“循环继续”。这是对图的解释。
# [原第 22 行] 说明文字：核心循环不断把工具结果反馈给模型。本行和后面三行合起来是一段英文说明。
# [原第 23 行] 说明文字：当模型返回的这一轮不再含工具调用时，停止继续调用模型；随后提到 WorkBuddy 的工程化实现。
# [原第 24 行] 说明文字：实际应用会在基础循环之外添加配套运行机制，原文称为 harness。
# [原第 25 行] 说明文字：其他机制将在后续章节讲解。此文件本身没有展示这些章节。
# [原第 27 行] Usage 表示“使用方式”，下方是供人参考的终端命令。
# [原第 28 行] 终端安装命令：pip install 安装 anthropic 和 python-dotenv 两个第三方包。它在字符串内部，因此运行此文件并不会自动安装依赖。
# [原第 29 行] POSIX shell 风格的启动示例：先为这次进程设置 API 密钥和模型名，再运行脚本。... 是要替换的占位符；这条写法不能原样当成 PowerShell 命令。
# [原第 30 行] 结束第 3 行开启的多行字符串。三个双引号必须成对，字符串中的换行和普通字符都属于文本内容。
"""
